...
export class SortListItems_Transaction extends jsTPS_Transaction {
    constructor(listId, column, sortFunc, restoreFunc) {
        super();
        this.listId = listId
        this.column = column
    ...
    async doTransaction(){
        const { data } = await this.sortFunc({
            variables: {_id: this.listId, column: this.column}
        });

        this.oldListIds = [];
        data.sortItems.map(id => {
            this.oldListIds.push(id);
        })
    ...
    async undoTransaction(){
        const { data } = await this.restoreFunc({
            variables: {_id: this.listId, oldList: this.oldListIds}
        });
