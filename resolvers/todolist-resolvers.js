	...
	Mutation: {
		...
		**/
		...
		sortItems: async (_, args) => {
			...
			listItems.sort((a,b) => (a[column] >= b[column]) ? 1 : -1);
			if (JSON.stringify(listItems) === JSON.stringify(oldItems)){
				listItems.sort((a,b) => (a[column] <= b[column]) ? 1 : -1);
			}
			const updated = await Todolist.updateOne({_id: listId}, {items: listItems});
			...
			let oldIndexMap = []
			oldItems.map(item => {
				oldIndexMap.push(item._id);
			});
			return oldIndexMap;
		},

		restoreList: async (_, args) => {
			...
			for(let i = 0; i < oldList.length; i++){
				for(let j = 0; j < listItems.length;j++){
					...
					if(listItems[j]._id.toString() === oldList[i].toString()){
						restoredList.push(listItems[j]);
					}
				}
			}

			const updated = await Todolist.updateOne({_id: listId}, {items: restoredList});
